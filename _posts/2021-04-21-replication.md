---
title: "DB 부하 분산을 위한 윈도우 환경 MySQL replication 설정"
...
---
## 데이터 복구 문제
제가 현재 진행 중인 프로젝트는 하나의 MySQL로 모든 읽기와 쓰기 작업을 담당하고 있습니다. 
...
두 데이터베이스는 서비스의 INSERT, UPDATE, DELETE 같은 쓰기 기능을 담당하는 마스터 서버와 SELECT 같은 읽기 기능 만을 담당하는 슬레이브 서버로 이뤄져 있습니다.. 
마스터 서버는 변경 내역을 로그로 기록한 다음 해당 로그를 슬레이브 서버로 보내 변경된 데이터를 업데이트합니다.
